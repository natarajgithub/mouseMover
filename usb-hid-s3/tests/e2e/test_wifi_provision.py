...
Flow under test:
  ...
  2. `radio wifi` with no creds → Soft-AP `usb-hid-s3-setup` + HTTP :80.
...
def test_wifi_clear_then_soft_ap(provision):
    ...
    assert "usb-hid-s3-setup" in m.line
...
def test_soft_ap_rest_configures_wifi(provision):
    ...
    time.sleep(0.5)
    harness.clear_buffer()
    harness.send_and_wait("radio wifi", r"soft-ap ssid=", timeout=15.0)

    _join_network("usb-hid-s3-setup")
    try:
        ...
        assert body.get("ap_ssid") == "usb-hid-s3-setup"
        ...
        pytest.fail(f"Soft-AP REST failed (is the Mac on usb-hid-s3-setup?): {e}")
    finally:
